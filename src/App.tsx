...
// Import the new sub-path landing page components
import CdrLandingPage from './components/CdrLandingPage';
import DpsLandingPage from './components/DpsLandingPage';
import SboLandingPage from './components/SboLandingPage';
import SaiLandingPage from './components/SaiLandingPage';
...
export default function App() {
  ...
  useEffect(() => {
    const handleLocationChange = () => {
      ...
      // Mendukung routing berbasis hashtag (misalnya: lynkidhub.online/#/cdr atau /#/dps)
      // Ini sangat berguna jika hosting tidak memiliki konfigurasi URL rewrite server-side.
      if (window.location.hash) {
        const hashPath = window.location.hash.substring(1); // Hapus tanda '#'
    ...
    // Dengarkan event perubahan history popstate & perubahan hash
    window.addEventListener('popstate', handleLocationChange);
    window.addEventListener('hashchange', handleLocationChange);

    // Lakukan verifikasi path secara berkala untuk stabilitas dalam iframe/preview
  ...
  // Routing bersyarat untuk masing-masing path
  if (currentPath === '/cdr' || currentPath === '/cdr/') {
    return <CdrLandingPage />;
  }

  if (currentPath === '/dps' || currentPath === '/dps/') {
    return <DpsLandingPage />;
  }

  if (currentPath === '/sbo' || currentPath === '/sbo/') {
    return <SboLandingPage />;
  }

  if (currentPath === '/sai' || currentPath === '/sai/') {
    return <SaiLandingPage />;
  }
